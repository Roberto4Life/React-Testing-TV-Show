...
test('dropdown renders', async () => {
    ...
    expect(queryAllByText(/select a season/i)).toHaveLength(0)

    await wait();

    // await waitForElement(() => {
    //     expect(queryAllByText(/Stranger Things/i)).toHaveLength(2); //toHaveLength(4)
    // })

    fireEvent.click(getByText(/select a season/i));

    expect(getByText(/seasons/i)).toHaveLength(4);
